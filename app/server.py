...
Thin by design: every endpoint is a shim over `src/agent/tools.py` or
`src/serving/engine.py`, so the API and the assistant cannot drift apart - they call
the same functions.
...
To use an LLM brain instead of the built-in router, set a key in the environment
before starting - never in the repo:

    setx MISTRAL_API_KEY "..."      # Windows, new shell afterwards
    export MISTRAL_API_KEY="..."    # bash
"""
