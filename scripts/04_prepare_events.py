"""Phase 1 / step 4 - filter + type the NYC Permitted Events export.

The published file covers 2008-onwards (~745 MB); we keep only rows overlapping the
project window and store them as UTC.

Important: the export carries **no latitude/longitude** - only a free-text street
description plus borough / community board / police precinct. Mapping events to H3
cells is therefore a geocoding problem, and it is deliberately deferred to Phase 4
(see docs/DATA_SOURCES.md). This script's job is to produce the clean, deduplicated
list of events and the inventory of distinct location strings that geocoding will
have to resolve.

    python scripts/04_prepare_events.py [--force]
"""
