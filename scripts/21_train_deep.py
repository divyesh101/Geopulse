"""Phase 6 - train the ST-GNN or the compact TFT from the exported tensor bundle.

Runs identically on a laptop CPU and a Kaggle GPU; only `--device` differs.

    python scripts/21_train_deep.py --model stgnn --epochs 6
    python scripts/21_train_deep.py --model tft   --epochs 6 --device cuda

The two models consume the same information, in the shape each one needs:

* **ST-GNN** predicts every region at once from a `[B, T, N, F]` window, so the
  spatial graph is used directly. One sample = one timestamp = 1,483 predictions.
* **TFT** predicts one region's series from a `[B, T, F]` window, splitting inputs
  into *observed* (past demand, weather) and *known-future* (calendar), which is the
  distinction the architecture is built around.
"""
