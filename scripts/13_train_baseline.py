"""Phase 3 - Seasonal Naive + one simple LightGBM per (target, horizon).

Eight LightGBM models (pickup/dropoff x h1..h4), no tuning. The gate:
**LightGBM must beat the better Seasonal Naive variant on validation MAE.**
If it does not, stop and investigate rather than adding features.

The TEST split is never read here. It is opened once, in Phase 7.

    python scripts/13_train_baseline.py --dev-sample
    python scripts/13_train_baseline.py
"""
