"""Phase 1 / step 7 - pull NYC DOT traffic speeds from the Socrata API.

Replaces the manual CSV export, which was row-capped to 2018-07-26..30.

Fetches day by day against the indexed `data_as_of` column, writes typed UTC
Parquet partitioned by month, and is **resumable**: days already written are
skipped unless `--force` is given, so an interrupted run just carries on.

    python scripts/07_fetch_traffic.py                    # full 2023-2024 pull
    python scripts/07_fetch_traffic.py --dev-sample       # just the 7-day dev window
    python scripts/07_fetch_traffic.py --registry-only    # rebuild the link registry
    SOCRATA_APP_TOKEN=... python scripts/07_fetch_traffic.py   # higher rate limit
"""
