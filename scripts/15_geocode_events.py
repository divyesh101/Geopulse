"""Phase 4 - geocode the NYC permitted-event location strings.

The events export carries no coordinates, only free text. Two shapes dominate:

* park facilities - ``Astoria Park: Soccer-01`` (the youth/adult sport permits,
  ~80% of rows)
* street segments - ``BROADWAY between WEST 43 STREET and WEST 44 STREET``

Both are normalised before querying NYC Planning Labs GeoSearch (free, no key).
**The borough is appended to the query and then validated against the response**:
without it, "McCarren Park" in Brooklyn resolves to a Bronx park. A result whose
borough disagrees with the permit is marked unresolved rather than used - a
confidently wrong coordinate is worse than a missing one, because Phase 4 would
attribute the event to the wrong H3 cell and the ablation would measure noise.

Resumable: already-geocoded strings are skipped unless --force.

    python scripts/15_geocode_events.py [--workers 8] [--limit N] [--force]
"""
