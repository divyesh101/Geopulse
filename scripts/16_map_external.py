"""Phase 4 - map events, traffic and station/network context onto (region, bin).

Three joins that each need a decision recorded rather than assumed:

**Events.** Geocodes resolve a location to a point; the event is attributed to the
containing H3 cell and its first ring, because a permit on a cell boundary affects
both sides. Expansion into 15-minute bins is capped (`--max-event-hours`) so a
multi-month permit cannot explode into millions of rows or drown the genuine
short-lived spikes the feature is meant to capture.

**Traffic.** A link is attributed to the cell containing its midpoint. Congestion is
measured against a per-link free-flow reference (the 85th percentile speed)
**computed on TRAIN rows only** - deriving it from the whole series would leak the
validation and test periods into a training feature.

**Station/network.** Computed per (region, month) from `first_seen`, so a station only
counts once it has actually appeared. Monthly granularity is deliberate: the counts
move slowly, and a per-bin version costs 100M rows for no extra signal.

    python scripts/16_map_external.py [--resolution 9]
"""
