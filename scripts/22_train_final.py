"""Phase 6 Step 1 - finalise LightGBM on the ablation-chosen feature set.

Two things Phase 3 left on the table, both fixed here:

* **Objective choice is tested, not assumed.** Poisson vs. L1 regression is compared
  on validation and the winner is kept.
* **The round cap actually binds.** Every Phase 3 model hit 500 rounds without early
  stopping firing, so those numbers were a floor. The cap is raised and early
  stopping is allowed to decide.

Optional bounded Optuna search (`--tune`), deliberately small: the ablation showed
the feature set matters far more than hyperparameters here, so an exhaustive sweep
would spend hours to move the third decimal.

    python scripts/22_train_final.py --tune --trials 15
    python scripts/22_train_final.py            # fixed params, all 8 targets
"""
