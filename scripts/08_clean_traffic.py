"""Phase 1 / step 8 - profile and clean the traffic observations.

Same discipline as the trip cleaner: profile the raw feed first, attribute every
removal to a named rule, then write a single Parquet file sorted by `data_as_of`
and verify the ordering.

    python scripts/08_clean_traffic.py
    python scripts/08_clean_traffic.py --dev-sample
"""
