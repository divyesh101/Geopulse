"""Phase 1 / step 2 - data-quality report, cleaning rules, global chronological sort.

Order of operations is deliberate:
  1. profile the RAW data and write the quality report  (before anything is deleted)
  2. de-duplicate ride ids, then attribute every remaining row to the first
     cleaning rule it violates
  3. write the surviving rows sorted by `started_at` to a single Parquet file
  4. verify sortedness explicitly (row-group stats + a full pairwise scan)

    python scripts/02_clean.py                 # full run
    python scripts/02_clean.py --dev-sample    # 7-day window only -> data/dev_sample
    python scripts/02_clean.py --report-only   # regenerate the report, skip the sort
"""
