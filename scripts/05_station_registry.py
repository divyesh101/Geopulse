"""Phase 1 / step 5 - build the station registry from actual trip activity.

Per docs/DATA_SOURCES.md the published roster is a *snapshot* of whatever date it
was pulled, so it cannot define which stations existed during 2023-2024. The
registry is therefore derived from observed trips; the roster is loaded only as a
cross-check and its coordinates are reported, never substituted.

Representative coordinates are the **median** of every observed appearance, which is
robust to the occasional GPS outlier. Stations whose observed coordinates span more
than `station_registry.coord_jump_km_threshold` are flagged, not dropped.

    python scripts/05_station_registry.py [--dev-sample]
"""
