"""Phase 2 gate check - verify the Definition of Done against the built panel.

Like the Phase 1 gate, this reads the artifacts rather than trusting the build log.
The target-alignment check re-derives every horizon by an independent timestamp
self-join on a random sample of regions across the whole window, because a silent
off-by-one there would corrupt every model from Phase 3 onward.

    python scripts/11_phase2_check.py [--resolution 9] [--sample-regions 60]
"""
