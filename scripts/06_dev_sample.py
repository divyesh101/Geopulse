"""Phase 1 / step 6 - carve the 7-day dev sample every later stage is verified on.

Built by slicing the **already-cleaned, already-sorted** full artifacts, so the dev
sample is guaranteed to have gone through identical rules - no second code path.

Weather and events are sliced with a configurable warm-up pad in front of the
window, because lag/rolling features in Phases 3-4 need history *before* the first
forecast timestamp.

    python scripts/06_dev_sample.py [--warmup-days 8]
"""
