"""Phase 7 - the single, final evaluation on the TEST split (Nov-Dec 2024).

**This is the only script that reads TEST.** Everything before it selected models on
validation; opening the holdout more than once would turn it into a second validation
set and quietly inflate every number in the report.

Produces:
  * core metrics (MAE / RMSE / WAPE) per target at the reported horizons
  * Hotspot precision / recall / F1 at each horizon
  * breakdowns: hour of day, weekday vs weekend, demand tercile, rain vs dry
  * a model comparison matrix including efficiency (inference time, artifact size)
  * error analysis: the largest errors, categorised

MAPE is absent on purpose - most cells are zero, and every workaround for that
changes what is being measured.

    python scripts/23_evaluate_test.py
"""
