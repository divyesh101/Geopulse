"""Phase 4 - build the full feature table (families A-H) for the ablation.

Per-region-series features (lags, rolling, EWM, trend, seasonal, expanding) are
computed in polars over a dense `(region_id, ts)` frame; the external context
(weather, events, traffic, station/network) and the spatial-neighbour aggregates are
joined in DuckDB, which is where the cross-region work belongs.

Batched by region for the same reason as Phase 3: lags and rolling windows are
per-region, so a region batch is exact and bounds peak memory. Neighbour aggregates
need the batch's neighbours loaded too, which the SQL handles.

Weather join rule: the latest hourly reading with `weather_timestamp <= forecast_time`,
and `forecast_time` is the END of bin `ts` (see docs/FORECAST_TIME_CONVENTION.md), so
the hour is taken from `ts + interval`. Staleness is 0-45 minutes, never negative.

    python scripts/17_build_features_full.py --dev-sample
    python scripts/17_build_features_full.py
"""
