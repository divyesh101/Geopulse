"""Phase 4 - the mandatory A-H ablation.

Adds feature families cumulatively and reports what each one actually buys:

    A: recent historical demand      E: + events
    B: + seasonal/calendar           F: + traffic
    C: + rolling/trend               G: + spatial neighbour
    D: + weather                     H: + station/network

**Reduced but identical.** The full 8-target x 8-family grid is 64 fits (~5 hours).
The ablation runs on h1 (pickup and dropoff) with a fixed row sample and round budget
that is *the same at every step*, which is what makes the steps comparable. The
winning feature set is then trained at full strength separately. Absolute MAEs here
are therefore slightly worse than the headline model; the deltas are the result.

    python scripts/18_ablation.py --dev-sample
    python scripts/18_ablation.py
"""
