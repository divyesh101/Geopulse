"""Phase 1 / step 1 - ingest raw Citi Bike CSVs into typed, UTC Parquet.

    python scripts/01_ingest.py                # full 2023-2024 run
    python scripts/01_ingest.py --limit 2      # smoke test on the first 2 files
    python scripts/01_ingest.py --force        # re-ingest files already written

No rows are dropped here; see 02_clean.py for the cleaning rules.
"""
