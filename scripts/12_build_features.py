"""Phase 3 - panel + basic features -> the model-ready feature table.

Calendar, cyclical encodings and plain lags only. Rolling / seasonal / weather /
events / traffic / neighbour families belong to Phase 4.

The first `max(basic_lags)` steps of every region (one week) are dropped: their lags
are null because there is no history yet, not because demand was zero.

    python scripts/12_build_features.py --dev-sample
    python scripts/12_build_features.py
"""
