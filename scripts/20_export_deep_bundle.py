"""Phase 6 - export a compact tensor bundle for the TFT and ST-GNN.

The deep models do not need the 5.9 GB feature table. They need the demand series
itself plus the known-future calendar, the observed weather, and the region graph -
about 400 MB, which fits comfortably in a Kaggle dataset so the GPU work can run
there while the LightGBM/DuckDB phases stay local.

Layout (all aligned on the same time axis, no gaps):

    demand.npy      int16   [T, N, 2]  pickups, dropoffs per timestep per region
    calendar.npy    float32 [T, C]     known-future: cyclical time, weekend, holiday
    weather.npy     float32 [T, W]     observed at forecast_time (already lagged)
    edges.npy       int32   [2, E]     first-ring adjacency, both directions
    node_static.npy float32 [N, S]     per-region constants (area, station count...)
    meta.json                          region ids, timestamps, split boundaries, scaling

**Scaling statistics are computed on TRAIN only** and written into meta.json, so the
Kaggle side cannot accidentally fit them on validation or test.

    python scripts/20_export_deep_bundle.py [--spatial h3] [--resolution 9]
"""
