"""Phase 1 gate check - verify every Definition-of-Done item against the artifacts.

Reads what was actually produced and reports PASS/FAIL per item, so the checklist in
`docs/STATUS.md` is evidence-backed rather than asserted. Exits non-zero if any item
fails, which makes it usable as the phase gate itself.
