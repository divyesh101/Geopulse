"""Phase 5 - H3 resolution study, S2 level matching, and the H3-vs-S2 comparison.

Research questions 1 and 2. **The model is held constant** (one untuned LightGBM
config) and so is the feature set (the Phase 3 basic features), so any difference is
attributable to the spatial representation rather than to modelling choices.

Part A  H3-8 / H3-9 / H3-10, identical pipeline and identical training budget.
Part B  Pick the S2 level whose granularity best matches the winning H3 resolution,
        by a weighted log-ratio distance on BOTH median cell area and active-cell
        count. Log-ratio because "twice as big" and "half as big" should cost the
        same; raw differences would bias the choice toward the larger level.
Part C  Run the same pipeline on the matched S2 level and compare accuracy AND
        efficiency - a representation can win on MAE and lose on practicality.

    python scripts/19_spatial_experiment.py --part A
    python scripts/19_spatial_experiment.py --part B
    python scripts/19_spatial_experiment.py --part C --s2-level 15
"""
