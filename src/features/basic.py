"""Phase 3 basic features: calendar, cyclical encodings, and plain lags.

Everything here is computable at `forecast_time` (= the row's own `ts`):

* **Calendar** features describe `ts` itself. A calendar value is known in advance,
  so reading it at `ts` is not leakage.
* **Lags** look strictly backwards: `pickups_lag_k[r,t] = pickups[r, t - k*interval]`.
  They are computed with SQL `LAG` over a **dense** grid, which is what makes the
  step count equal a fixed wall-clock offset. `tests/test_features.py` re-derives
  every lag by an independent timestamp join rather than trusting that.

Rolling stats, EWMs, trend, seasonal-slot expectations, weather, events, traffic and
neighbour features are deliberately **not** here - they belong to Phase 4. The gate
is supposed to measure a clean baseline, not a head start.
"""
