"""Phase 4 feature families, as polars expressions, grouped for the A-H ablation.

Why polars here and DuckDB in Phase 3: exponentially-weighted means have no SQL
equivalent, and rolling/expanding windows over a sorted per-region series are what
polars is best at. The frame handed to these builders is always sorted by
`(region_id, ts)` and dense, which is what makes `.over("region_id")` equal a fixed
wall-clock offset.

**Leakage rule** (see `docs/FORECAST_TIME_CONVENTION.md`): `forecast_time` is the end
of bin `ts`, so bin `ts` is the last *completed* bin and its demand is observable.
Rolling and EWM windows therefore legitimately include the current row. Anything that
would read `ts + 1` or later is forbidden and is asserted against in the tests.
"""
