"""Cleaned trips -> a dense (region x time-bin) demand panel with horizon targets.

Shape of the output, one row per active region per interval:

    region_id, ts (UTC), pickups, dropoffs, net_flow, dst_unreliable,
    pickup_h1..h4, dropoff_h1..h4

Design notes
------------
* **Density is the contract.** A region with no trips in a bin has demand 0, not a
  missing row. Everything downstream - lags, rolling windows, the LEAD-based targets
  below - is only correct on a gap-free grid, so the grid is built explicitly as a
  cross join and the trip counts are joined onto it, never the other way round.
* **Bins are UTC.** Phase 1 stores tz-aware UTC; local calendar features are derived
  later by converting back. Since US offsets are whole hours and the interval divides
  an hour, the UTC grid and the local grid align exactly.
* **Targets use LEAD, which is only valid because the grid is dense.**
  `pickup_h{h}[r,t] = pickups[r, t + h*interval]`. A test asserts this against an
  independent timestamp self-join rather than trusting the row ordering.
* **DST-distorted bins are flagged, not silently modelled.** Phase 1 measured that
  the second pass through the repeated fall-back hour is empty by construction and
  the first is over-filled. `dst_unreliable` marks those bins so Phase 3+ can exclude
  or down-weight them instead of learning a fake nightly collapse.
"""
