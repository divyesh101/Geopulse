"""One forecast entry point for every model, fast enough to sit behind a UI.

`src/serving/predict.py` is the Phase 8 contract and stays as written. This is the
app's engine: same numbers, but it keeps boosters, tensor bundles and region maps in
memory instead of re-reading them per call, and it serves the two deep models, which
`predict()` does not.

**Why the deep models need their own path.** LightGBM consumes one row of the feature
table. ST-GNN and TFT consume a *window* of history as a tensor - 24 steps for the
graph model, 96 for the transformer - plus same-time-yesterday and same-time-last-week
slices. Those come from the exported bundle, not the feature parquet, so the two
paths genuinely differ and are kept separate rather than forced into one signature.

All four models are scored on the same instant and the same regions, so the app can
