"""A real walkable street graph, so the route follows pavements instead of cutting
across blocks.

`src/serving/routing.py` chooses *which* dock to send you to. This module answers
*how you actually walk there* - and it is the piece that was previously missing, so
routes were drawn as straight lines between docks.

Nodes and edges come from OpenStreetMap via the Overpass API: every `highway` in a
small bounding box, minus motorways and anything tagged `foot=no` or access-
restricted. Each consecutive pair of nodes in a way becomes a bidirectional edge
weighted by its true ground distance. The result is fed to the **same A\\*** in
`routing.py` - identical algorithm, real edges.

Two deliberate choices:

* **Fetched per request for a small box, then cached on disk.** The whole five
  boroughs would be a few hundred MB and stale the moment it downloaded; a 2 km box
  around one trip is ~1.3 MB and arrives in about four seconds. Subsequent routes in
  the same area are served from cache.
* **It degrades instead of failing.** No network, an Overpass rate limit, or a
  point with no mapped streets nearby all fall back to the straight-line station
  path, flagged so the UI can say which one the user is looking at.
"""
