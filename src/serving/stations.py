"""Station layer: map stations to regions, and split a region forecast across them.

The models forecast **regions**, not stations - 321 H3-8 cells holding ~7.7 stations
each. The app is asked about stations, so a region forecast has to be divided up.

The split is each station's share of its own region's lifetime trips, taken from
`station_registry.parquet` (2,459 stations, two full years). A station that
historically took 18% of its cell's pickups is assigned 18% of the cell's forecast.

**This is an approximation and every caller must say so.** It assumes a station's
share of its neighbourhood is stable across the day, which is not exactly true - a
station outside an office building and one outside a bar sit in the same cell and
swap roles between 09:00 and 22:00. Modelling stations directly would fix that and
costs a full re-run of Phases 2-6 at 2,459 regions. `share_confidence` flags the
cases where the assumption is weakest, so the UI can hedge rather than mislead.
"""
