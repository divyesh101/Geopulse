"""Context and explanation: why the forecast says what it says.

**The honesty rule this module exists to enforce.** The Phase 4 ablation rejected
weather, events, traffic and station-network features twice - at H3-9 and again at
H3-8, where traffic coverage was four times better. The deployed model uses only
families A+B+C: recent demand, calendar/seasonality, and rolling trends.

So an explanation is allowed to say *"this cell is running 30% above its usual
Tuesday 5pm level"* - the model really used that. It is **not** allowed to say
*"demand is up because of the street fair"*, because the model never saw the street
fair and the ablation measured its effect at -0.01%.

Nearby events are still worth showing - people want to know - so they are returned
under `context`, clearly separated from `drivers`, and carry `influences_forecast:
False`. Anything rendering this must keep that separation.
"""
