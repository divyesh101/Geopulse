"""Phase 8 - the standardized prediction interface.

One function, one row schema, whatever the model or spatial system:
...
Backend-only by design - it returns a polars DataFrame, not JSON or HTTP - so a UI,
a batch job and a test all share exactly one code path.

Two conventions it inherits rather than re-invents:

* `forecast_time` is the END of bin `ts` (docs/FORECAST_TIME_CONVENTION.md). The
  features behind a forecast made at `forecast_time` therefore live on the row whose
  `ts` is `forecast_time - interval`, and that row is fully observed. A
  `forecast_time` off the bin grid is an error, not a silent rounding.
* `projected_inventory` accumulates over horizons 1..h, so asking for horizon 4 runs
  1-4 and sums them. Shortage and surplus are measured against that projection -
  the point of the layer is that a region which is fine now but empties within the
  hour should surface now.

The inventory it starts from is an ESTIMATE (src/inventory/estimate.py); Citi Bike
publishes no historical dock occupancy. Every row carries `inventory_estimated` so a
caller cannot mistake it for a measurement.
"""
