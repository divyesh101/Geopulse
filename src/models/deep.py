"""Phase 6 deep models: a compact TFT and a spatio-temporal GNN.

Both read the tensor bundle produced by `scripts/20_export_deep_bundle.py`, so this
module runs unchanged on a laptop CPU or a Kaggle GPU - only `--device` changes.

**Why hand-written rather than pytorch-forecasting / torch-geometric.** Both packages
pin narrow torch versions and pull in a lightning stack; on a 4 GB laptop GPU and a
Kaggle image that is a lot of surface area for two models whose core is a few hundred
lines. The TFT below implements the paper's actual components - variable selection
over gated residual networks, an LSTM encoder, interpretable multi-head attention,
static enrichment - not a generic seq2seq wearing the name. The GNN implements graph
attention directly against the edge list.

**Fairness rule (Phase 6).** Both models see the same *categories* of information as
LightGBM: past demand, calendar, weather, spatial structure, static region stats. No
model receives a value timestamped after `forecast_time`.
"""
