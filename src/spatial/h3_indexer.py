"""H3 implementation of :class:`~src.spatial.base.SpatialIndexer`.

Bulk point assignment runs in DuckDB via the `h3` community extension; the Python
methods here are for the small-cardinality work (neighbours, centroids, boundaries
over the ~10^3 active regions). Both paths must agree - `tests/test_spatial.py`
checks them against each other on real coordinates.
"""
