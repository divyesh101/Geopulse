"""Raw Citi Bike CSV -> typed, timezone-correct Parquet.

This stage does **no filtering**. It only parses, types and localizes, so that the
data-quality report in `02_clean.py` describes the data as published, and any row
we later drop is attributable to a named cleaning rule.
"""
