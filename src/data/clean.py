"""Cleaning rules, chronological sort, and the sortedness verification.

Design notes
------------
* Every removal is attributed to the **first** rule a row violates (`drop_reason`),
  so the waterfall counts sum exactly to the rows removed - no silent drops.
* Rules that describe a row rather than invalidate it (missing station id,
  round trip) become **flag columns**, not deletions.
* Ride-id de-duplication is handled *before* the rule pass, by materialising only
  the rows that share a ride id. That set is tiny (zero rows in 2023-2024), so the
  rest of the pipeline never carries a 79M-row window function.
* The output is one Parquet file sorted by `started_at`. A single global ORDER BY
  over 79M wide rows spills tens of GB and does not finish in a sane time; because
  the raw data is already partitioned by `started_at` month, sorting each month
  (~3M rows, in memory) and concatenating the parts **in month order** produces the
  identical global ordering for a fraction of the cost. `verify_sorted` proves it
