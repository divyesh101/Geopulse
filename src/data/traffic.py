"""NYC DOT real-time traffic speeds (Socrata dataset i4gi-tjb9).

Shape of this source, and why it is fetched the way it is
---------------------------------------------------------
Each row is one speed reading for one *road link* at one instant: roughly one
reading per link every five minutes, ~122 links, ~23.8M rows across 2023-2024.

Every row also repeats the link's full geometry (`link_points`, a long
whitespace-separated lat,lng string, plus an encoded polyline). That geometry is
**static per link**, so carrying it on 23.8M rows inflates the download by roughly
an order of magnitude - which is what made the manual CSV export unusable. We
therefore split the source in two:

* **observations** - `link_id, speed, travel_time, status, data_as_of` only.
* **link registry** - one row per link with geometry, sampled across the window so
  a link whose geometry changed is still captured.

Pagination is by **time window**, not `$offset`: `data_as_of` is indexed, whereas
deep offsets degrade badly over 23.8M rows and cannot be resumed cleanly.

`data_as_of` is a Socrata *floating* timestamp - naive local New York wall-clock,
exactly like the Citi Bike timestamps - so it goes through the same
:mod:`src.data.timezone` localization and is stored as tz-aware UTC.
"""
