"""Cleaning rules and profiling for the traffic observation table.

Reuses the same machinery as the trip cleaner (`drop_reason_case`, `drop_waterfall`)
so traffic gets the same discipline: every removal named, counted and reconciled.

Two things about this feed drive the rules:

* **`status` is the feed's own validity marker.** `0` means a real reading; anything
  else (`-101` in practice, ~20% of rows) means the sensor reported nothing. Those
  rows carry `speed = 0`, which would silently drag any average toward zero if kept.
* **`id` is not a row id.** It is the link's own small integer identifier - there are
  as many distinct `id` values as links. The natural key of an observation is
  `(link_id, data_as_of)`, and that is what de-duplication uses.
"""
