"""Timezone handling for Citi Bike / weather timestamps.

Citi Bike publishes naive *local wall-clock* timestamps for New York. Two things go
wrong if you treat them as naive strings:

* **Spring forward** (2023-03-12, 2024-03-10): local 02:00-02:59 does not exist.
  Any such timestamp is a data error; we shift it forward by one hour and count it.
* **Fall back** (2023-11-05, 2024-11-03): local 01:00-01:59 happens twice. Binning
  naive local time silently merges two distinct real hours into one, inflating that
  hour's demand and creating a phantom gap.

Everything downstream is therefore stored as **tz-aware UTC**, which is unambiguous
and keeps the 15-minute grid dense across both transitions. Local calendar features
are derived by converting back with :func:`to_local`.
"""
