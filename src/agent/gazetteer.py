"""Turn "I'm near Times Square" into a coordinate, using only our own data.

No geocoding API. Two sources we already built:

* **2,459 station names** from the registry - almost all NYC intersections
  ("W 21 St & 6 Ave"), which is how people describe where they are in Manhattan.
* **19,011 geocoded event locations** from Phase 4 - parks, plazas, venues and
  landmarks, each already resolved to a coordinate with a borough check.

Together they cover street corners and landmarks, which is most of what a rider
would type. Matching is deliberately simple and transparent: normalise, then score
on token overlap with a substring bonus. Every result carries the source and score
so the UI can show *what* it matched rather than silently teleporting the user.
"""
