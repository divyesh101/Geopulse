"""The assistant: two brains, one set of tools.

**Claude mode** - if the `anthropic` package is installed and `ANTHROPIC_API_KEY` is
set, questions go to Claude with the tools in `src/agent/tools.py` exposed for
tool-use. Handles paraphrase, follow-ups and anything unanticipated.

**Router mode** - otherwise, a deterministic keyword router calls the same tools and
renders the same cards. No key, no network, no cost. It cannot hold a conversation,
but it answers the questions this app exists to answer, which keeps the demo working
on a fresh clone.

Both return the identical payload shape, so the frontend never knows which ran.

The system prompt carries the project's honesty rules, because the failure mode for
an LLM here is inventing a causal story - "demand is up because of the parade" - for
a feature the ablation explicitly rejected. The tools attach that caveat to every
result; the prompt tells the model it may not discard it.
"""
