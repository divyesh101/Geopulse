"""The tools the assistant can call. Each returns plain JSON-able dicts.

Design rule: **a tool never narrates.** It returns numbers plus the metadata needed
to describe them honestly - what was modelled, what was derived, what is only
context. The language model does the wording; it cannot invent the caveats because
they arrive attached to the data.

Two facts every tool carries forward:

* Station numbers are **derived** from a region forecast by historical share, not
  modelled directly. `approximate: True` says so.
* Availability is a **flow-pressure signal**, not a dock count - no historical dock
  occupancy exists for 2023-24.
"""
