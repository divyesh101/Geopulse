"""Phase 8 - station capacity and daily inventory reconstruction.

**This is an approximation, and the code says so in its output.** No historical dock
count exists for 2023-2024 (see docs/DATA_SOURCES.md), so capacity is inferred from
observed flow and inventory is reconstructed from it. Every artifact carries a
`capacity_source` label and every reconstructed day carries an `inventory_fit_error`,
so nothing downstream can mistake this for measured truth.

Method
------
Let ``F(t) = cumulative(dropoffs - pickups)`` within one day at one station.

*Capacity*: the q95 of the daily range ``max(F) - min(F)`` over all days. A station
must hold at least the largest swing it actually absorbed; the 95th percentile trims
the tail where staff clearly intervened.

*Starting inventory*: for a day to be feasible with no rebalancing, the start level
``s`` must satisfy ``s + F(t)`` in ``[0, C]`` for every t, i.e.
``s in [max(0, -min F), min(C, C - max F)]``. Where that interval is non-empty we
take the point closest to half capacity; where it is empty the day could not have
happened without intervention, so we solve a small 1-D problem minimising
below-zero + overflow + distance-from-half and record the residual as
``inventory_fit_error``.

**Days are reconstructed independently and never chained.** Carrying a level across
two years would compound every error and every unobserved van movement.
"""
