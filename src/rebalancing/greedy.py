"""Phase 8 - shortage/surplus detection and the greedy rebalancer, plus its evaluation.

The point of the phase is not the optimiser. It is the comparison: simulate the same
future demand **with and without** forecast-driven rebalancing and report whether the
forecast actually bought anything operationally. A model with slightly worse MAE can
still win here, so both metric families are reported side by side and neither is
